...
export const CAM_END_Z = -(artworks.length - 1) * SLAB_SPACING - 4.5; // just past last slab
...
export const SLABS: SlabPlacement[] = sorted.map((work, i) => {
  ...
  // radius breathes between ~4.6 and ~6.4 so the corridor never feels mechanical
  const radius = 5.4 + Math.sin(i * 2.39996) * 0.9;
...
/** Which slab is nearest a given camera z (slab reads best ~4.4 units ahead). */
export function nearestSlabIndex(camZ: number): number {
  const i = Math.round((-camZ + 4.4) / SLAB_SPACING);
...
export const APP_NODES: AppNode[] = APPS_RAW.map((app, k) => {
  // between slab clusters: offset half a golden turn from the local slabs,
  // tighter to the axis so they read as waypoints, not artworks
  const t = (k + 0.75) / APPS_RAW.length;
  const angle = (t * helixSpan / SLAB_SPACING) * GOLDEN_ANGLE + Math.PI;
  const radius = 2.5 + (k % 2) * 0.7;
  return {
    ...app,
